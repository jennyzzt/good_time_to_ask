...
class ResnetTensorObjectNavAskActorCritic(ActorCriticModel[CategoricalDistr]):
    def __init__(
        # base params
        self,
        action_space: gym.spaces.Discrete,
        ...
        if (
            ...
            self.goal_visual_encoder = ResnetDualTensorGoalAskEncoder(  # type:ignore
                self.observation_space,
                goal_sensor_uuid,
                rgb_resnet_preprocessor_uuid,
                depth_resnet_preprocessor_uuid,
...
class ResnetDualTensorGoalAskEncoder(nn.Module):
    def __init__(
        self,
        observation_spaces: SpaceDict,
        goal_sensor_uuid: str,
    ...
    def forward(self, observations):
        ...
        rgbd_uuids = ["rgb_resnet", "depth_resnet", "goal_object_type_ind"]
...
class ResnetTensorObjectNavAskNLPActorCritic(ActorCriticModel[CategoricalDistr]):
    def __init__(
        # base params
        self,
        action_space: gym.spaces.Discrete,
        ...
        rgb_resnet_preprocessor_uuid: Optional[str] = None,
        depth_resnet_preprocessor_uuid: Optional[str] = None,
        nlp_preprocessor_uuid: Optional[str] = None,
        goal_dims: int = 32,
        resnet_compressor_hidden_out_dims: Tuple[int, int] = (128, 32),
        combiner_hidden_out_dims: Tuple[int, int] = (128, 32),
    ):
        ...
        if (
            ...
            self.goal_visual_encoder = ResnetDualTensorGoalAskNLPEncoder(  # type:ignore
                self.observation_space,
                goal_sensor_uuid,
                rgb_resnet_preprocessor_uuid,
                depth_resnet_preprocessor_uuid,
                nlp_preprocessor_uuid,
                goal_dims,
                resnet_compressor_hidden_out_dims,
                combiner_hidden_out_dims,
            )
...
class ResnetDualTensorGoalAskNLPEncoder(nn.Module):
    def __init__(
        self,
        observation_spaces: SpaceDict,
        goal_sensor_uuid: str,
        ...
        self.rgb_resnet_uuid = rgb_resnet_preprocessor_uuid
        self.depth_resnet_uuid = depth_resnet_preprocessor_uuid
        self.nlp_uuid = nlp_preprocessor_uuid
        self.class_dims = class_dims
        self.resnet_hid_out_dims = resnet_compressor_hidden_out_dims
        self.combine_hid_out_dims = combiner_hidden_out_dims
        self.embed_class = nn.Embedding(
        ...
        if not self.blind:
            ...
            # self.nlp_compressor = nn.Sequential(
            #     nn.Conv2d(self.resnet_tensor_shape[0], self.resnet_hid_out_dims[0], 1),
            #     nn.ReLU(),
            #     nn.Conv2d(*self.resnet_hid_out_dims[0:2], 1),
            #     nn.ReLU(),
            # )
            self.nlp_compressor = RobertaModel.from_pretrained("roberta-base", output_hidden_states=True)
            for param in self.nlp_compressor.base_model.parameters():
                param.requires_grad = False
            self.nlp_compressor.eval()
            # self.nlp_compressor = BertModel.from_pretrained("bert-base-uncased", output_hidden_states=True)
            self.rgb_target_obs_combiner = nn.Sequential(
                nn.Conv2d(
                    self.resnet_hid_out_dims[1] + self.class_dims,
                    self.combine_hid_out_dims[0],
            ...
            self.nlp_target_obs_combiner = nn.Sequential(
                # nn.Conv2d(
                #     self.resnet_hid_out_dims[1] + self.class_dims,
                #     self.combine_hid_out_dims[0],
                #     1,
                ...
                # nn.ReLU(),
                # nn.Conv2d(*self.combine_hid_out_dims[0:2], 1),
                nn.Linear(768*4 + self.class_dims, 32*7*7),
                nn.ReLU(),
            )
    ...
    def compress_nlp(self, observations):
        nlp_obs = observations[self.nlp_uuid]
        attention_mask = nlp_obs > 0
        hidden_states = self.nlp_compressor(input_ids=nlp_obs, attention_mask=attention_mask.int()).hidden_states
        last_4_hidden_states = torch.permute(torch.stack(hidden_states[-4:]), (1,0,2,3))[:,:,0,:]
        last_4_hidden_states = torch.flatten(last_4_hidden_states, 1)
        return last_4_hidden_states
    ...
    def adapt_input(self, observations):
        rgb = observations[self.rgb_resnet_uuid]
        depth = observations[self.depth_resnet_uuid]
        nlp = observations[self.nlp_uuid]
        ...
        observations[self.rgb_resnet_uuid] = rgb.view(-1, *rgb.shape[-3:])
        observations[self.depth_resnet_uuid] = depth.view(-1, *depth.shape[-3:])
        observations[self.nlp_uuid] = nlp.view(-1, nlp.shape[3])
        observations[self.goal_uuid] = observations[self.goal_uuid].view(-1, 1)
    ...
    def forward(self, observations):
        ...
        depth_x = self.depth_target_obs_combiner(torch.cat(depth_embs, dim=1,))
        nlp_embs = [
            self.compress_nlp(observations),
            self.embed_class(observations[self.goal_uuid]).view(-1, self.class_dims),
        ]
        nlp_x = self.nlp_target_obs_combiner(torch.cat(nlp_embs, dim=1,))
        # flatten before concatenation for NLP
        rgb_x = torch.flatten(rgb_x, 1)
        depth_x = torch.flatten(depth_x, 1)
        x = torch.cat([rgb_x, depth_x, nlp_x], dim=1)
        x = x.reshape(x.size(0), -1)  # flatten

        rgbd_uuids = ["rgb_resnet", "depth_resnet", "goal_object_type_ind", "nlp"]
        other_obs = [v.reshape(x.size(0), -1) for (k, v) in observations.items() if k not in rgbd_uuids]
        other_x = torch.cat(other_obs, dim=1)
        x = torch.cat([x, other_x], dim=1)
...
class ResnetTensorObjectNavAskDirActorCritic(ActorCriticModel[CategoricalDistr]):
    def __init__(
        # base params
        self,
        action_space: gym.spaces.Discrete,
        ...
        if (
            ...
            self.goal_visual_encoder = ResnetDualTensorGoalAskDirEncoder(  # type:ignore
                self.observation_space,
                goal_sensor_uuid,
                rgb_resnet_preprocessor_uuid,
                depth_resnet_preprocessor_uuid,
...
class ResnetDualTensorGoalAskDirEncoder(nn.Module):
    def __init__(
        self,
        observation_spaces: SpaceDict,
        goal_sensor_uuid: str,
...
class ClipTensorObjectNavAskActorCritic(VisualNavActorCritic):
    def __init__(
        # base params
        self,
        action_space: gym.spaces.Discrete,
        ...
        auxiliary_uuids: Optional[List[str]] = None,
        # custom params
        rgb_clip_preprocessor_uuid: Optional[str] = None,
        depth_resnet_preprocessor_uuid: Optional[str] = None,
        goal_dims: int = 32,
        clip_compressor_hidden_out_dims: Tuple[int, int] = (128, 32),
        combiner_hidden_out_dims: Tuple[int, int] = (128, 32),
    ):
        super().__init__(
            action_space=action_space,
        ...
        if (
            rgb_clip_preprocessor_uuid is None
            or depth_resnet_preprocessor_uuid is None
        ):
            pass  # not used case
        else:
            self.goal_visual_encoder = ClipDualTensorGoalAskEncoder(  # type:ignore
                self.observation_space,
                goal_sensor_uuid,
                rgb_clip_preprocessor_uuid,
                depth_resnet_preprocessor_uuid,
                goal_dims,
                clip_compressor_hidden_out_dims,
                combiner_hidden_out_dims,
            )
        self.create_state_encoders(
            obs_embed_size=self.goal_visual_encoder.output_dims,
...
class ClipDualTensorGoalAskEncoder(nn.Module):
    def __init__(
        self,
        observation_spaces: SpaceDict,
        goal_sensor_uuid: str,
        rgb_clip_preprocessor_uuid: str,
        depth_resnet_preprocessor_uuid: str,
        class_dims: int = 32,
        clip_compressor_hidden_out_dims: Tuple[int, int] = (128, 32),
        combiner_hidden_out_dims: Tuple[int, int] = (128, 32),
        ...
        super().__init__()
        self.goal_uuid = goal_sensor_uuid
        self.rgb_clip_uuid = rgb_clip_preprocessor_uuid
        self.depth_resnet_uuid = depth_resnet_preprocessor_uuid
        self.class_dims = class_dims
        self.clip_hid_out_dims = clip_compressor_hidden_out_dims
        self.combine_hid_out_dims = combiner_hidden_out_dims
        self.embed_class = nn.Embedding(
            num_embeddings=observation_spaces.spaces[self.goal_uuid].n,
            embedding_dim=self.class_dims,
        ...
        self.blind = (
            self.rgb_clip_uuid not in observation_spaces.spaces
            or self.depth_resnet_uuid not in observation_spaces.spaces
        )
        if not self.blind:
            self.clip_tensor_shape = observation_spaces.spaces[
                self.rgb_clip_uuid
            ].shape
            self.rgb_clip_compressor = nn.Sequential(
                # nn.Flatten(),
                # nn.Linear(self.clip_tensor_shape[0], self.clip_hid_out_dims[1]),
                # nn.ReLU(),
                # nn.Unflatten(1, (self.clip_hid_out_dims[-1], 1, 1)),
                nn.Conv2d(self.clip_tensor_shape[0], self.clip_hid_out_dims[0], 1),
                nn.ReLU(),
                nn.Conv2d(*self.clip_hid_out_dims[0:2], 1),
            )
            self.depth_resnet_compressor = nn.Sequential(
                nn.Conv2d(self.clip_tensor_shape[0], self.clip_hid_out_dims[0], 1),
                nn.ReLU(),
            ...
            self.rgb_target_obs_combiner = nn.Sequential(
                nn.Conv2d(
                    self.clip_hid_out_dims[1] + self.class_dims,
                    self.combine_hid_out_dims[0],
                    1,
                ),
                nn.ReLU(),
    ...
    def output_dims(self):
        if self.blind:
            ...
            return (
                2
                * self.combine_hid_out_dims[-1]
                * self.clip_tensor_shape[1]
                * self.clip_tensor_shape[2]
                + 2
            )
    ...
    def compress_rgb_clip(self, observations):
        return self.rgb_clip_compressor(observations[self.rgb_clip_uuid])
    ...
    def distribute_target(self, observations):
        target_emb = self.embed_class(observations[self.goal_uuid])
        return target_emb.view(-1, self.class_dims, 1, 1).expand(
            -1, -1, self.clip_tensor_shape[-2], self.clip_tensor_shape[-1],
        )

    def adapt_input(self, observations):
        rgb = observations[self.rgb_clip_uuid]
        depth = observations[self.depth_resnet_uuid]
        ...
        observations[self.rgb_clip_uuid] = rgb.view(-1, *rgb.shape[-3:])
        observations[self.depth_resnet_uuid] = depth.view(-1, *depth.shape[-3:])
        observations[self.goal_uuid] = observations[self.goal_uuid].view(-1, 1)
    ...
    def forward(self, observations):
        ...
        if self.blind:
            return self.embed_class(observations[self.goal_uuid])
        rgb_embs = [
            self.compress_rgb_clip(observations),
            self.distribute_target(observations),
        ]
        rgb_x = self.rgb_target_obs_combiner(torch.cat(rgb_embs, dim=1,))
        depth_embs = [
        ...
        rgbd_uuids = ["rgb_clip", "depth_resnet", "goal_object_type_ind"]
        other_obs = [v.reshape(x.size(0), -1) for (k, v) in observations.items() if k not in rgbd_uuids]
        other_x = torch.cat(other_obs, dim=1)
        x = torch.cat([x, other_x], dim=1)
